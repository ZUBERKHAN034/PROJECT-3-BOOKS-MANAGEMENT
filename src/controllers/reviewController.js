...
const { isValidReqBody, isValid, isValidObjectId, isValidRating, isValidName } = require("../validator/validation");
...
const createReview = async function (req, res) {
  try {
    ...
    if (!isValidObjectId(bookId)) {
      return res.status(400).send({ status: false, message: "You are entering invalid bookId." });
    }
    ...
    if (reviewData.hasOwnProperty("reviewedBy")) {
      ...
      if (!isValidName(reviewedBy)) {
        return res.status(400).send({ status: false, message: "plesae give a valid reviewedBy name" });
      }
    ...
    if (reviewData.hasOwnProperty("review")) {
      if (!isValid(review)) {
        return res.status(400).send({ status: false, message: "plesae give a valid  book review" }); //review is must
      }
    }
    ...
    //wrapping response body(the property we want send to view in response)
    ...
    const findReviewId = await reviewModel.findById(reviewCreated._id)
    ...
    const udatedBookReview = await bookModel.findOneAndUpdate({ _id: bookId }, { $inc: { reviews: 1 } });
...
const reviewUpdate = async function (req, res) {
  try {
    ...
    //updating review
    const updatedReview = await reviewModel.findOneAndUpdate({ _id: reviewId }, { ...bodyFromReq }, { new: true });

    const allAvailableReviews = await reviewModel.find({ bookId: bookId, isDeleted: false })
      .select({ isDeleted: 0, createdAt: 0, updatedAt: 0, __v: 0 });

    //setting a new key in review object
    let booksWithReviews = availableBook.toJSON();
    booksWithReviews.reviewData = allAvailableReviews;

    return res.status(200).send({ status: true, data: booksWithReviews });
  ...
  try {
    ...
    if (review.isDeleted === false && book.isDeleted === false) {
      ...
      if (review.bookId.toString() !== bookId) {
        return res.status(400).send({ status: false, message: `review is not from ${book.title}` });
      }
