...
const { isValid, isValidName, isValidPhone, isValidEmail, isValidPincode, isValidPassword, isValidReqBody } = require("../validator/validation");
...
const registerUser = async function (req, res) {
  try {
    ...
    if (isValid(title) === false) {
      return res.status(400).send({ status: false, message: "Please enter title(required field)" });
    }
    // if title is invalid
    ...
    // if phone is empty
    if (!isValid(phone)) {
      return res.status(400).send({ status: false, message: "Please enter the phone number(required field)" });
    }
    ...
    // if email is empty
    if (!isValid(email)) {
      return res.status(400).send({ status: false, message: "Please Enter email(required field)" });
    }
    ...
    let emaildb = await userModel.findOne({ email: email }, { email: 1, _id: 0 });
    if (emaildb) {
      return res.status(400).send({ status: false, message: "We are sorry; this email is already used" });
    }

    // is password is empty
    if (!isValid(password)) {
      return res.status(400).send({ status: false, message: " Please enter password(required field)" });
    }
    ...
    if (!isValidPassword(password)) {
      ...
      return res.status(400).send({ status: false, message: `password cannot be ${length}` });
    }
...
const loginUser = async function (req, res) {
  try {
    ...
    let token = jwt.sign(
      { email: login.email.toString(), userId: login._id }, "Group14", { expiresIn: "24h" }
    );

    // JWT generated sent back in response header
    res.setHeader("x-api-key", token);

    res.status(200).send({ status: true, message: "Login Successfull! Token sent in header 'x-api-key'" });
